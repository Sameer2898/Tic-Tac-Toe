...
def playerMove():
    run = True
    while run:
        move = input(r'Please select a position(1-9) where you want to place "X" :- ')
        try:
            move = int(move)
            if move > 0 and move < 10:
                if spaceIsFree(move):
                    ...
                    print('This place is occupied by AI. Please select another one.')
            else:
                print('Please enter a number with the range(1-9).')
        except:
    ...
    while not(isBoardFull(board)):
        ...
        if not(isWinner(board, 'X')):
            ...
            if move == 0:
                ...
                insertLetter('O', move)
                print('AI places an "O" in position', move, ':')
                printBoard(board)
        else:
            print('X Is Won The Game. You Beat The AI.')
            break
